...
export function touchStreak(state: StreakState, now = new Date()): StreakState {
  ...
  if (!state.lastActiveDate) return { current: 1, longest: Math.max(1, state.longest), lastActiveDate: today };

  const deltaDays = Math.round((dayNumber(today) - dayNumber(state.lastActiveDate)) / 86_400_000);
