...
export function readMastery(map: Record<string, MasteryRecord>, lessonId: string): MasteryRecord {
...
export function recordAttempt(
  ...
  const accuracy = next.attempts === 0 ? 0 : next.successes / next.attempts;
  const cleanRunBonus = hintUsed ? 0 : outcome === 'success' ? 0.12 : 0;
  const hintPenalty = hintUsed ? 0.04 : 0;
  const failurePenalty = outcome === 'success' ? 0 : 0.10;
  const nextMastery = Math.max(0, Math.min(1, current.mastery * 0.72 + accuracy * 0.28 + cleanRunBonus - hintPenalty - failurePenalty));

  next.mastery = Number(nextMastery.toFixed(3));

  const remediation = nextMastery < 0.45 && next.attempts >= 2;
  const difficultyDelta: -1 | 0 | 1 = remediation ? -1 : nextMastery > 0.82 && next.attempts >= 2 ? 1 : 0;
  ...
  return {
    mastery: { ...map, [lessonId]: next },
    signal: { mastery: next.mastery, difficultyDelta, remediation, message },
  };
}

export function recommendedHintIndex(record: MasteryRecord, hintCount: number): number {
...
export function masteryPercent(record: MasteryRecord): number {
  return Math.round(record.mastery * 100);
}
