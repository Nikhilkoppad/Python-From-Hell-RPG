...
function test(pattern:RegExp,source:string){pattern.lastIndex=0;const matched=pattern.test(source);pattern.lastIndex=0;return matched}
export async function evaluateLessonChallenge(lesson:Lesson,source:string,visible:RunResult,runner:PythonRunner):Promise<ChallengeResult>{
 ...
 for(const pattern of spec.required??[])if(!test(pattern,source))return{passed:false,reason:`Your output is close, but the solution is missing required concept: ${pattern.source}`};
 for(const pattern of spec.forbidden??[])if(test(pattern,source))return{passed:false,reason:`This solution uses a shortcut the challenge forbids: ${pattern.source}`};
 if(spec.harness){
