...
export function playSfx(kind: 'enter' | 'success' | 'fail' | 'boss' | 'achievement') {
  ...
  const notes = {
    ...
    fail: [130, 90],
    boss: [82, 110, 73],
    achievement: [392, 523, 659],
  }[kind];
  osc.type = kind === 'fail' ? 'sawtooth' : 'square';
  osc.frequency.setValueAtTime(notes[0], now);
  notes
    .slice(1)
    .forEach((freq, index) => osc.frequency.setValueAtTime(freq, now + (index + 1) * 0.09));
