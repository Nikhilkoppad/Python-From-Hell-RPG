...
async function runtime(){
 if(!pyodidePromise)pyodidePromise=import(PYODIDE_URL).then(async mod=>mod.loadPyodide({indexURL:INDEX_URL}));
...
self.onmessage=async event=>{
 try{
  ...
  try{await py.runPythonAsync(String(event.data?.code??''));self.postMessage({stdout:out.trimEnd(),stderr:err.trimEnd()})}
